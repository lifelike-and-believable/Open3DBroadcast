...
Initial skeleton description send
Per-frame transform + curve updates
Handling skeleton changes (re-send description)
...
Curve support: CURVE_SUPPORT.md
Test Code
...
Planning Context
Open3DBroadcast_Planning.md (M0 section)
Success Metrics
Completeness: All message types and fields relevant to broadcast are documented
Clarity: A new developer can understand protocol structure from the reference doc alone
Validation: Reference doc has been reviewed against actual message examples
Reusability: Document serves as a guide for implementing M1+ tasks
